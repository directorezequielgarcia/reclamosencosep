    ...
    <div className="flex flex-col gap-6">
      <header>
        ...
        <p className="text-sm text-muted mt-1">
          Convocatorias del Ente con inscripción ciudadana en línea.
        </p>
      </header>
      ...
      <section className="rounded-2xl border border-line bg-paper p-5">
        ...
        <form action={crearAudiencia} className="grid sm:grid-cols-2 gap-3">
          ...
          <Field label="Descripción *" full>
            <textarea name="descripcion" required rows={3} className="px-3 py-2 rounded-lg border border-line-strong text-sm bg-paper resize-y" />
          </Field>
          <Field label="Fecha y hora *">
            <input name="fecha" type="datetime-local" required className="px-3 py-2 rounded-lg border border-line-strong text-sm bg-paper" />
          </Field>
          <Field label="Modalidad *">
            <select name="modalidad" required defaultValue="" className="px-3 py-2 rounded-lg border border-line-strong text-sm bg-paper">
              <option value="" disabled>Seleccionar…</option>
              {(Object.keys(MODALIDAD_META) as ModalidadAudiencia[]).map((m) => (
                <option key={m} value={m}>{MODALIDAD_META[m].icon} {MODALIDAD_META[m].label}</option>
              ))}
            </select>
          </Field>
          <Field label="Lugar (si es presencial)">
            <input name="lugar" type="text" placeholder="ej: Sala del Concejo Deliberante" className="px-3 py-2 rounded-lg border border-line-strong text-sm bg-paper" />
          </Field>
          <Field label="Enlace virtual (Zoom/Meet)">
            <input name="enlaceVirtual" type="url" placeholder="https://…" className="px-3 py-2 rounded-lg border border-line-strong text-sm bg-paper" />
          </Field>
          <Field label="Capacidad (opcional)">
            <input name="capacidad" type="number" min={1} className="px-3 py-2 rounded-lg border border-line-strong text-sm bg-paper" />
          </Field>
                ...
                <li key={a.id}>
                  ...
                  >
                    <div className="flex items-center gap-2 flex-wrap text-[11px]">
                      ...
                      <span className="text-muted">{MODALIDAD_META[a.modalidad].label}</span>
                      <span className="text-muted">·</span>
                      ...
                      <span className="ml-auto font-bold text-navy">{a._count.inscripciones} inscripto{a._count.inscripciones === 1 ? "" : "s"}</span>
                    </div>
