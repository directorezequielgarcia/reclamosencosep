...
import { ROL_LABEL, puedeVerConsulta, puedeVerConsultaReclamos } from "@/lib/admin";
import { SubmitButton } from "@/components/ui/SubmitButton";

// Secciones de solo lectura (indicadores, encuesta, reclamos ingresados) a
// las que se llega desde las tarjetas de "Reportes"/"Fiscalización" del
// panel institucional (app/institucional/page.tsx) — no un panel aparte.
// Deliberadamente separado de /admin — ese panel tiene navegación y datos
// (bandeja completa, WhatsApp, documentación) que no corresponde exponer
// fuera del equipo del Ente.
export default async function ConsultaLayout({
  ...
  if (!session) redirect("/ingresar?callbackUrl=/institucional");
    ...
    <div className="flex flex-1 flex-col w-full min-h-screen bg-paper-2">
      <header className="bg-navy text-white print:hidden">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center gap-6">
          <Link href="/institucional" className="flex items-center gap-3">
